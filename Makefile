...
define macos_build
	...
	cp $(WHISPER_CPP_DIR)/$(BUILD_PATH)/src/libwhisper.dylib $(RUNTIME_DIR)/macos-$(ARCH)$(2)/
	cp $(WHISPER_CPP_DIR)/$(BUILD_PATH)/ggml/src/libggml-whisper.dylib $(RUNTIME_DIR)/macos-$(ARCH)$(2)/
endef
